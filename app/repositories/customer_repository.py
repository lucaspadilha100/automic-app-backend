...
class TenantCustomerRepository:
    """Small repository with tenant-safe helpers for TenantCustomer."""

    model = TenantCustomer

    def get_by_id(self, db: Session, tenant_id: UUID, entity_id: UUID) -> Optional[TenantCustomer]:
        return db.query(self.model).filter(self.model.id == entity_id, self.model.tenant_id == tenant_id).first()

    def list_by_tenant(self, db: Session, tenant_id: UUID, skip: int = 0, limit: int = 100) -> List[TenantCustomer]:
